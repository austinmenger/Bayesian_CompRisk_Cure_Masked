...
c......in the Presence of Cure Fractions
...
c       real*8  sumC(5) !added for cure DEV
       ...
       real*8  ss(5), h(5), cpr(5), fbetaC(5), einL(5)
...
!      number of warm-up iterations
       nbin=2500 !2500 to fully run, was 100 for tuning and first paper submission
c      nbin=25
!      number of thinning
       nthin=5 !5 to fully run, was 1 for tuning and first paper submission
...
c......initialize surv and cure params
...
c......read the number of Gibbs iterates
       nrep=5000 !5000 to run, was 250 for tuning and 500 for original paper submission
...
c!         do 110 i2=1,n
           if(ts(i2) .gt. ts(id1)) then !i2 = i' and id1 = i in notation
            comp(k)=comp(k)+1.0d0 
            sum1=0.0d0 
            ss2=0.0d0
...
142       continue
          ...
          prodint(k)= fbetaC(k)+(1.0d0-fbetaC(k))
     1         *dexp(ss(k))
673      enddo
         ...
         do 143 k=1,5
c          kp=3-k !k prime
          cprnum1=(1.0d0-fbetaC(k))*dexp(h(k))*dexp(ss(k))
...
c          enddo
          do kin=1,5
           cprdenom1=(1.0d0-fbetaC(kin))*dexp(h(kin))
     1         *dexp(ss(kin))
...
!         if (d(i1) .eq. ak) then 
           if (int(d(i1)) .eq. 0) then
            sumNC(k)= sumNC(k)+dlog(fbetaC(k)
     1       + (1.0d0-fbetaC(k))*dexp(ss(k)))
           else 
            if (int(d(i1)) .eq. k) then
             sumNC(k)=sumNC(k)+dlog(1.0d0-fbetaC(k))
     1        +h(k)+ss(k)
            else 
              sumNC(k)=sumNC(k)+dlog(fbetaC(k)
     1         + (1.0d0-fbetaC(k))*dexp(ss(k)))
...
c     1     + dlog(fbetaC(k)+(1.0d0-fbetaC(k))*dexp(ss(k)))*cpr(k) !kprime goes to other four k's
          sumNC(k)=sumNC(k)+(dlog(1.0d0-fbetaC(k))+h(k)+ss(k))*cpr(k)
     1     +4.0d0*dlog(fbetaC(k)+(1.0d0-fbetaC(k))*dexp(ss(k)))*cpr(k) !kprime goes to other four k's
         endif
!........WAIC
...
c......compute DICsurv (replace ebetaC, ebeta, ealam, etc...)
...
c           cpr(k)=cprnum - dlog(cprdenom) !log of conditional prob
     ...
     1       + (1.0d0-fbetaC(k))*dexp(ss(k)))
             ...
             if (int(d(i1)) .eq. k) then
              sumNC(k)=sumNC(k)+dlog(1.0d0-fbetaC(k))+h(k)+ss(k)
             else 
               sumNC(k)=sumNC(k)+dlog(fbetaC(k)
     1         + (1.0d0-fbetaC(k))*dexp(ss(k)))
             endif 
           ...
           if (dold(i1) .eq. 6.0d0) then !the case where u_i = 1 (no log)
            sumNC(k)=sumNC(k)+(dlog(1.0d0-fbetaC(k))+h(k)+ss(k))
     1       *cpr(k)+4.0d0*dlog(fbetaC(k)+(1.0d0-fbetaC(k))
     1       *dexp(ss(k)))*cpr(k) !special b/c separate
...
522      continue
     ...
     1      +sumNC(5))
      ...
      open(unit=12,file='BSOoutput.out',!"Bayes Surv Only"
     1       access='sequential',status='unknown')
...
c      include 'GaussianHermite.f'
       include 'gibbsBSO.f' ! newest gibbs code (was 'gibbsJointNew_mc_newalgo.f')
